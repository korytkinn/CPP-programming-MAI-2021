...
class Sorter
{
    ...
    int* massiv;
    int* massivSecond; //если старый заполнили элементами полностью
...
public:
    ...
    Sorter() // конструктор по умолчанию
    {
        massiv = nullptr;
        massivSecond = nullptr;
    }
    ~Sorter()
    {
    ...
    Sorter(const Sorter& copy)
    {
        ...
        oldSize = copy.oldSize;
        massiv = new int[oldSize]; //задаем размер массива

        for (int i = 0; i < copy.MassiveLength; i++)
        {
            massiv[i] = copy.massiv[i];
        }

        MassiveLength = copy.MassiveLength;
        MassivePosition = copy.MassivePosition;
        number = copy.number;
        oldSize = copy.oldSize;
        ...
        matrix = new int* [rowCount]; // rowCount строки в массиве
        for (int count = 0; count < (rowCount); count++)
            matrix[count] = new int[columnCount]; // и columnCount столбцов
        ...
        maxPartIndikator = copy.maxPartIndikator;
        deltaIndikator = copy.deltaIndikator;
        partsCount = copy.partsCount;

        for (int p = 0; p < maxPart; p++)
        {
            stroka1[p] = copy.stroka1[p];
            stroka2[p] = copy.stroka2[p];
        }
    } //конец оператора копирования

    // Перегрузка оператора присваивания
    Sorter& operator= (const Sorter& copy)
    {
    ...
    // Конструктор копирования
    Sorter(const Sorter &copy) :
        massiv(copy.massiv), massivSecond(copy.massivSecond), MassiveLength(copy.MassiveLength), MassivePosition(copy.MassivePosition),
        number(copy.number), oldSize(copy.oldSize), Jump(copy.Jump), matrix(copy.matrix), rowCount(copy.rowCount), columnCount(copy.columnCount),
        maxPartIndikator(copy.maxPartIndikator), deltaIndikator(copy.deltaIndikator), partsCount(copy.partsCount), stroka1(copy.stroka1), stroka2(copy.stroka2)
    ...
    */
    ...
    bool check(int* a1, int* a2, int maxPart)
    {
        ...
        if (sameIndikator == 0) //если совпавших чисел >1 или = 0, то возвращаем false
        {
            return false;
        }
        ...
        razmer1 = 0;
        razmer2 = 0;
        for (int i = 0; i < columnCount; i++) // проверка по столбцам
        {
            if (a1[i] != 0)
            {
                razmer1++;
    ...
    int deltaCalculator(int* a1, int* a2, int maxPart) // калькулятор
    {
        int sum1 = 0, sum2 = 0;
        for (int i = 0; i < maxPart; i++)
        {
    ...
    void display() //много логики сделать позже
    {
        ...
        for (int i = 0; i < rowCount; i++)
        {
            for (int j = 0; j < columnCount; j++)
            {
                cout << matrix[i][j] << " "; // выводит массив
    ...
    void creator()
    {
        ...
        while (number != 0)
        {
            ...
            {
                massivSecond = new int[oldSize + Jump]; //задаем размер массива
...
//        int realSize = 0;
        /*for (int i = 0; i < oldSize; i++) //считаем число элементов
        {
            //            cout << massiv[i] << " ";
            if (massiv[i] != 0)
            {
        ...
        int sizeSqrt = mySqrt(MassiveLength); // кв.корень из длины массива
        rowCount = sizeSqrt;
    ...
    void devider()
    {
        ...
        // нужно одну из строк записать первой строкой, другую первым столбцом. columnCount rowCount

        // Логика, если columnCount == rowCount
        /*if (columnCount == rowCount)
        {
            for (int i = 0; i < columnCount; i++)
            {
                ...
                {
                    matrix[0][i] = 0; // забиваем строку первую
                }
                if (stroka2[i] != 0)
                {
                    matrix[i][0] = massiv[stroka2[i] - 1]; //забиваем первый столбец
                }
                else
                {
                    matrix[i][0] = 0; //забиваем первый столбец
                }
