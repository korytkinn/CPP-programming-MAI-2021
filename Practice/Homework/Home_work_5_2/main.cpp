...
int mySqrt(int x)
{
    ...
    // Стартуем с 1, пробуем все цифры до
    ...
    bool check(int* a1, int* a2, int maxPart)
    {
        ...
        {
            //            return false;
            return false; // ПОЧЕМУ ЭТО ДАЕТ ПРАВИЛЬНЫЙ ОТВЕТ, ЕСЛИ НУЛИ НЕ СОВПАЛИ МЫ ГОВОРИМ ЧТО СОВПАЛИ! (при 5 числах)
        }
...
//        int sizeSqrt = sqrt(MassiveLength); // кв.корень из длины массива ОРИГИНАЛ
